...
| `bank_id_by_user` | — | Optional JSON object mapping an exact gateway `user_id` to a bank id. A matching entry overrides the static/template bank for that user; unmatched or malformed entries preserve the normal fallback. Useful when one profile serves multiple known users that require hard memory separation. Keep this mapping in the profile-scoped private config, not shared prompts or logs. |
