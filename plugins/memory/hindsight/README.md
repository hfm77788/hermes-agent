...
| `bank_id_by_user` | — | Optional JSON object mapping a platform-scoped gateway key (`<platform>:<user_id>`, e.g. `dingtalk:<id>`) to a bank id. A matching entry overrides the static/template bank for that user; unmatched or malformed entries preserve the normal fallback. Raw `user_id` keys are only used when no platform is available, preventing cross-platform ID collisions. Keep this mapping in the profile-scoped private config, not shared prompts or logs. |
